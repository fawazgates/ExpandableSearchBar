...
struct Home: View {
    ...
    @Environment(\.colorScheme) private var schame
    ...
    @ViewBuilder
    func expandablenavigationbar(_ title: String = "massages") -> some View {
                    ...
                    HStack(spacing: 12) {
                        ForEach(Tab.allCases, id: \.rawValue) { tab in
                            Button(action: {
                                ...
                                Text(tab.rawValue)
                                    .font(.callout)
                                    .foregroundStyle(activeTab == tab ? (schame == .dark ? .black : .white) : Color.primary)
                                    .padding(.vertical, 8)
